                ...
                <tbody>
                    ...
                    @foreach ($blogs as $key => $blog)
                        <tr>
                            ...
                            <td valign="middle">
                                <img src="@if (!empty($blog->image)){{asset('storage/blogs/' . basename($blog->image))}}@else{{asset('library/admin/default-image.png')}}@endif" alt="{{$blog->name}}" style="max-width: 100%; max-height: 100px;">
                            </td>
                            <td valign="middle">{{$blog->name}}</td>
                            <td valign="middle">{{$blog->created_at->format('d/m/Y');}}</td>
                            <td valign="middle" align="center">
                                <a href="{{route('edit_blog',[$blog->id])}}" class="btn btn-outline-info" title="Sửa"><i class="fa-solid fa-pen-to-square"></i></a>
                                <button class="btn btn-outline-danger" title="Xóa" onclick="deleteItem({{$blog->id}},'tin tức','{{route('delete_blog')}}');"><i class="fa-solid fa-trash"></i></button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
